...
export const useAppLovinAds = () => {
  ...
  const initializeSDK = useCallback(async () => {
    ...
    try {
      ...
      if (!consented) {
        console.warn("[useAppLovinAds]  UMP consent not yet ready; initializing anyway (consent may complete later)");
      ...
      // } catch (healthError) {
      // console.warn('[useAppLovinAds]  Backend health check failed:', healthError);
      ...
      if (token) {
        ...
        try {
          ...
          if (configResponse?.success && configResponse?.data) {
            ...
            config = {
              ...
              // Use our hardcoded ad unit ID, not the placement name from backend
            ...
            console.warn(
              "[useAppLovinAds]  Backend config response invalid:",
              {
        ...
        console.warn("[useAppLovinAds]  No auth token available");
      ...
      if (success) {
        ...
        appLovinPlugin.loadAd().catch((err) => {
          console.warn(
            "[useAppLovinAds]  Background preload failed:",
            err?.message,
          );
        });
  ...
  const loadAd = useCallback(async () => {
    ...
    try {
      ...
      if (token) {
        ...
        console.warn("[useAppLovinAds]  No auth token, skipping backend tracking");
      }
        ...
        if (token && currentAdRecordIdRef.current) {
          ...
          try {
            ...
            if (completionResponse?.success && completionResponse?.data) {
              ...
              console.warn("[useAppLovinAds]  Backend completion response invalid, using fallback");
            }
          } catch (completeError) {
            console.error(
          ...
          console.warn("[useAppLovinAds]  Cannot track completion:", {
            hasToken: !!token,
