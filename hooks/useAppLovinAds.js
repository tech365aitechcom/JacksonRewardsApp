...
export const useAppLovinAds = () => {
  ...
  const normalizeRevenue = useCallback((rev) => {
    ...
    if (rev && typeof rev === 'object') {
      const amount = typeof rev.amount === 'number' ? rev.amount : (typeof rev.value === 'number' ? rev.value : undefined);
  ...
  const setupListeners = useCallback(() => {
    ...
    appLovinPlugin.addLoadListener((adInfo) => {
      ...
      setIsAdReady(true);
      setIsLoading(false);
      console.log('[useAppLovinAds] 📈 State updated: isAdReady=true, isLoading=false');
    });
    ...
    appLovinPlugin.addCompleteListener((result) => {
      ...
      console.log('[useAppLovinAds] 📈 State updated: isShowingAd=false, isAdReady=false, lastReward set');
    });
    ...
    appLovinPlugin.addFailListener((error) => {
      ...
      console.log('[useAppLovinAds] 📈 State updated: isLoading=false, isShowingAd=false, error set');
    });
  ...
  const initializeSDK = useCallback(async () => {
    ...
    if (initializationAttemptedRef.current || isInitialized) {
      console.log('[useAppLovinAds] ⏭️ SDK already initialized or attempt in progress, skipping');
    ...
    try {
      ...
      let config = {
        sdkKey: '',
        adUnitId: APPLOVIN_CONFIG.AD_UNIT_ID.ANDROID_REWARDED, // Real ad unit ID: 6e87990ef5f63cce
        placement: 'rewarded',
      };
      ...
      if (token) {
        ...
        try {
          ...
          console.log('[useAppLovinAds] 📥 Backend config response received:', configResponse);
          console.log('[useAppLovinAds] 📊 Config data:', {
            success: configResponse?.success,
            hasData: !!configResponse?.data,
            sdkKey: configResponse?.data?.sdkKey ? '***' + configResponse.data.sdkKey.slice(-10) : 'missing',
          ...
          if (configResponse?.success && configResponse?.data) {
            ...
            config = {
              ...
              adUnitId: APPLOVIN_CONFIG.AD_UNIT_ID.ANDROID_REWARDED, // 6e87990ef5f63cce
              placement: configResponse.data.adUnitId || 'rewarded', // This is actually the placement
            };
            setSdkConfig({
              ...configResponse.data,
              actualAdUnitId: APPLOVIN_CONFIG.AD_UNIT_ID.ANDROID_REWARDED,
            });
            console.log('[useAppLovinAds] ✅ Using backend SDK key with actual ad unit ID');
            ...
            console.warn('[useAppLovinAds] ⚠️ Backend config response invalid:', {
              success: configResponse?.success,
              hasData: !!configResponse?.data,
            });
          ...
          console.error('[useAppLovinAds] ❌ Failed to fetch backend config:', configError);
        ...
        console.warn('[useAppLovinAds] 📝 Using default config (SDK may not work without token)');
      }

      // Initialize the plugin
      console.log('[useAppLovinAds] 🔍 Step 3: Initializing AppLovin plugin...');
      ...
      if (success) {
        ...
        // Set up listeners
        console.log('[useAppLovinAds] 🔍 Step 4: Setting up event listeners...');
        setupListeners();
        ...
        console.log('[useAppLovinAds] ✅ Initialization complete - ready to load ads');
        
        return true;
      } else {
        console.error('[useAppLovinAds] ❌ Plugin initialization returned false');
      ...
      console.log('[useAppLovinAds] 📈 State updated: error set, isLoading=false');
      return false;
    } finally {
      setIsLoading(false);
      console.log('[useAppLovinAds] 📈 State updated: isLoading=false (finally)');
    }
  ...
  const loadAd = useCallback(async () => {
    ...
    if (!isInitialized && !initializationAttemptedRef.current) {
      console.log('[useAppLovinAds] ⚠️ SDK not initialized, initializing first...');
      await initializeSDK();
    }

    setIsLoading(true);
    setError(null);
    console.log('[useAppLovinAds] 📈 State updated: isLoading=true, error cleared');
    ...
    try {
      ...
      console.log('[useAppLovinAds] 🔍 Step 1: Tracking ad load with backend...');
      if (token) {
        ...
        console.log('[useAppLovinAds] 📤 Sending load tracking data:', loadTrackingData);
        
        try {
          const loadResponse = await trackAppLovinAdLoad(loadTrackingData, token);
          console.log('[useAppLovinAds] 📥 Backend load response received:', loadResponse);
          ...
          if (loadResponse?.success && loadResponse?.data?.adRecordId) {
            ...
            console.log('[useAppLovinAds] ✅ Ad record created and stored:', currentAdRecordIdRef.current);
          } else {
            console.warn('[useAppLovinAds] ⚠️ Backend did not return adRecordId');
            console.warn('[useAppLovinAds] 📝 Response:', loadResponse);
          }
        } catch (backendError) {
          console.error('[useAppLovinAds] ❌ Backend load tracking failed:', backendError);
        ...
        console.warn('[useAppLovinAds] ⚠️ No auth token, skipping backend tracking');
      }

      // Load ad via plugin
      console.log('[useAppLovinAds] 🔍 Step 2: Loading ad via native plugin...');
      ...
      setIsAdReady(false);
      console.log('[useAppLovinAds] 📈 State updated: error set, isAdReady=false');
      
      // Track failure
      if (token) {
        console.log('[useAppLovinAds] 🔍 Tracking load failure with backend...');
        ...
        }, token).catch((trackError) => {
          console.error('[useAppLovinAds] ❌ Failed to track load failure:', trackError);
        });
      }
  ...
  const showAd = useCallback(async (options = {}) => {
    ...
    if (!isAdReady && pluginReady) {
      console.log('[useAppLovinAds] ✅ Plugin reports ad is ready (state race). Proceeding...');
      setIsAdReady(true);
    }

    setIsShowingAd(true);
    setError(null);
    console.log('[useAppLovinAds] 📈 State updated: isShowingAd=true, error cleared');

    try {
      // Track ad display with backend
      console.log('[useAppLovinAds] 🔍 Step 1: Tracking ad display with backend...');
      if (token && currentAdRecordIdRef.current) {
        ...
        try {
          ...
          console.error('[useAppLovinAds] ❌ Backend display tracking failed:', displayError);
      ...
      // Show ad via plugin
      console.log('[useAppLovinAds] 🔍 Step 2: Showing ad via native plugin...');
      console.log('[useAppLovinAds] 📤 Calling appLovinPlugin.showAd("rewarded")...');
      const result = await appLovinPlugin.showAd('rewarded');
      
      console.log('[useAppLovinAds] ✅ Ad show complete, result received:', result);
      ...
      console.log('[useAppLovinAds] 🔍 Step 3: Tracking ad completion with backend...');
      if (token && currentAdRecordIdRef.current) {
        const platformInfo = appLovinPlugin.getPlatformInfo();
        console.log('[useAppLovinAds] 📱 Platform info for completion:', platformInfo);
        ...
        try {
          ...
          console.log('[useAppLovinAds] 📋 Payload:', JSON.stringify(completionPayload, null, 2));
          
          const completionResponse = await trackAppLovinAdComplete(completionPayload, token);

          console.log('[useAppLovinAds] ✅ Completion tracked with backend');
          console.log('[useAppLovinAds] 📥 Completion response:', completionResponse);
          console.log('[useAppLovinAds] 📊 Response data:', {
          ...
          if (completionResponse?.success && completionResponse?.data) {
            const backendReward = {
              coins: completionResponse.data.coinsAwarded || result.reward?.amount || 100,
              xp: completionResponse.data.xpAwarded || 5,
              ...completionResponse.data,
            };
            console.log('[useAppLovinAds] 💰 Backend reward processed:', backendReward);
            setLastReward(backendReward);
            onReward?.(backendReward);
            console.log('[useAppLovinAds] 📈 State updated: lastReward set, onReward callback called');
            ...
            console.log('[useAppLovinAds] ✅ Ad show process complete with backend reward');
            return backendReward;
          } else {
            console.warn('[useAppLovinAds] ⚠️ Backend completion response invalid, using fallback');
          }
        } catch (completeError) {
          console.error('[useAppLovinAds] ❌ Backend completion tracking failed:', completeError);
      ...
      // Fallback reward
      console.log('[useAppLovinAds] 💰 Using fallback reward (no backend tracking)');
      const reward = {
      ...
      onReward?.(reward);
      console.log('[useAppLovinAds] 📈 State updated: lastReward set (fallback)');
      
      // Preload next ad
      console.log('[useAppLovinAds] 🔍 Preloading next ad...');
      preloadNextAd();
      
      console.log('[useAppLovinAds] ✅ Ad show process complete with fallback reward');
      return reward;
    } catch (err) {
      ...
      // Track failure
      if (token && currentAdRecordIdRef.current) {
        console.log('[useAppLovinAds] 🔍 Tracking display failure with backend...');
        await trackAppLovinAdFailure({
          adRecordId: currentAdRecordIdRef.current,
          adUnitId: 'rewarded', // Backend expects placement name
          ...
          console.error('[useAppLovinAds] ❌ Failed to track display failure:', trackError);
        });
      }
      ...
      console.log('[useAppLovinAds] 📈 State updated: isShowingAd=false, adRecordId cleared (finally)');
    }
