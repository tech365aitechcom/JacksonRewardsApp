  ...
  try {
    ...
    if (finalUrl && userId) {
      try {
        ...
        // Add user_id for Bitlab games (required by Bitlab for re-encryption and tracking)
        if (provider === "bitlab") {
        ...
        console.warn(
          "URL parsing failed, appending as query string:",
          urlError,
        );
      ...
      if (!userId) {
        console.warn("No user ID found - using original URL");
      }
      if (!finalUrl) {
    ...
    if (window.Capacitor && window.Capacitor.isNativePlatform()) {
      try {
        ...
        console.warn("Browser plugin not available, falling back to window.open");
      }
    }
