...
export const handleGameDownload = async (game) => {
  try {
    ...
    const { normalizeGameUrl, normalizeGameTitle, getSdkProvider } = require('@/lib/gameDataNormalizer');
    let finalUrl = normalizeGameUrl(game) || game.url || game.details?.downloadUrl || game.downloadUrl;
    ...
    if (finalUrl && userId) {
      try {
        ...
        console.log(`✅ Added partner_user_id to ${provider} URL:`, finalUrl);
      } catch (urlError) {
        ...
        console.log(`✅ Added partner_user_id and game_id to ${provider} URL (fallback):`, finalUrl);
      }
    } else {
      if (!userId) {
    ...
    const fallbackUrl = normalizeGameUrl(game) || game.url || game.details?.downloadUrl;
    if (fallbackUrl) {
