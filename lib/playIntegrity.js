...
const debugIntegrity = () =>
  DEBUG || (typeof localStorage !== "undefined" && localStorage?.getItem("debug_api") === "true");
const log = (...args) => {
...
export async function getIntegrityChallenge(authToken = null) {
  ...
  try {
    ...
    log("getIntegrityChallenge: raw data", data, "-> nonce:", nonce ? `${nonce.slice(0, 20)}...` : null);
    return nonce;
  } catch (e) {
...
export async function getIntegrityToken(options = {}) {
  ...
  log("getIntegrityToken: called", { hasAuthToken: !!authToken, googleCloudProjectNumber });
  ...
  log("getIntegrityToken: nonce source:", nonceSource, "nonce length:", nonce?.length, "preview:", nonce ? `${nonce.slice(0, 24)}...` : null);

  try {
    const { PlayIntegrity } =
      await import("@capacitor-community/play-integrity");
    log("getIntegrityToken: calling requestIntegrityToken", { nonceLength: nonce?.length, googleCloudProjectNumber });
    const result = await PlayIntegrity.requestIntegrityToken({
      nonce,
      googleCloudProjectNumber,
    });
    const token = result?.token ?? null;
    log("getIntegrityToken: SDK result", { hasToken: !!token, tokenLength: token?.length ?? 0 });
    return token;
  } catch (err) {
    log("getIntegrityToken: requestIntegrityToken failed", err?.message ?? err, err);
    if (
