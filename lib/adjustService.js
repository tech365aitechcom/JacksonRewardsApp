...
export const fetchDynamicTokens = async (authToken) => {
  ...
  try {
    ...
    if (cached) {
      ...
      if (age < CACHE_TTL_MS) {
        ...
        console.log(`[Adjust Dynamic] ✅ Using cached tokens (age: ${Math.round(age / 60000)}min) — SDK: ${Object.keys(_dynamicTokens).length} | S2S: ${Object.keys(_s2sTokenMap).length}`);
        return;
      }
      console.log("[Adjust Dynamic] 🔄 Cache expired — fetching fresh tokens from backend");
    }
  } catch {
  ...
  try {
    console.log("[Adjust Dynamic] 📡 Fetching tokens from /v2/adjust/events...");
    ...
    if (!res.ok) {
      console.warn(`[Adjust Dynamic] ❌ Backend returned ${res.status} — using hardcoded tokens`);
      return;
    }
    ...
    if (!Array.isArray(events) || events.length === 0) {
      console.warn("[Adjust Dynamic] ⚠️ No events returned from backend — using hardcoded tokens");
      return;
    }
    ...
    console.log(`[Adjust Dynamic] ✅ Tokens loaded — SDK: ${sdkMatched} | S2S: ${s2sMatched} | total events: ${events.length}`);
  } catch (err) {
    console.warn("[Adjust Dynamic] ❌ Fetch failed — using hardcoded tokens:", err?.message);
  }
};
...
const getEventType = (eventName) => {
  ...
  if (n.includes("purchase") || n.includes("revenue") || n.includes("withdrawal")) return "purchase";
...
const fireS2S = async (eventName, extraPayload = {}) => {
  ...
  if (!authToken) {
    console.warn(`[Adjust S2S] ⚠️ DROPPED — no auth token | event: ${eventName}`);
    return;
  }
  if (!eventName) return;
  ...
  if (!eventToken) {
    console.warn(`[Adjust S2S] ⚠️ No token found for "${eventName}" — tokens not loaded yet or event not in backend`);
    return;
  }

  try {
    const deviceId   = await getDeviceId();
    const deviceIdType = (typeof window !== "undefined"
    ...
    console.log(`[Adjust S2S] 📤 Firing — event: "${eventName}" | token: ${eventToken} | type: ${eventType} | deviceId: ${deviceId}`);
    ...
    fetch(`${BASE_URL}/api/v2/adjust/report-event`, {
      ...
      .then((res) => {
        if (res.ok) {
          console.log(`[Adjust S2S] ✅ Success — event: "${eventName}" | token: ${eventToken} | status: ${res.status}`);
        } else {
          console.warn(`[Adjust S2S] ❌ Failed — event: "${eventName}" | status: ${res.status}`);
        }
      })
      .catch((err) => {
        console.warn(`[Adjust S2S] ❌ Network error — event: "${eventName}"`, err?.message);
      });
  } catch (err) {
 ...
 */
const fireEvent = (sdkTokenKeyOrToken, s2sName, sdkParams = {}, s2sPayload = {}) => {
  ...
  if (_dynamicTokens) {
    const tokenKey = Object.keys(SDK_TOKENS).find(k => SDK_TOKENS[k] === sdkTokenKeyOrToken);
    if (tokenKey && _dynamicTokens[tokenKey]) {
      resolvedToken = _dynamicTokens[tokenKey];
    }
  }

  console.log(`[Adjust] 🔥 fireEvent — SDK token: ${resolvedToken || "none"} | S2S: ${s2sName || "none"} | source: ${_dynamicTokens ? "dynamic" : "hardcoded"}`);
...
const getGameDownloadKey = (n) => {
  const map = { 1:"gameDownload1st", 2:"gameDownload2nd", 3:"gameDownload3rd",
                4:"gameDownload4th", 5:"gameDownload5th", 10:"gameDownload10th",
                15:"gameDownload15th", 20:"gameDownload20th" };
  return map[n] || null;
};

const getSurveyKey = (n) => {
  const map = { 1:"survey1", 2:"survey2", 3:"survey3", 5:"survey5",
                10:"survey10", 15:"survey15", 20:"survey20",
                30:"survey30", 40:"survey40", 50:"survey50" };
  return map[n] || null;
};

const getDailyChallengeKey = (n) => {
  const map = { 2:"dailyChallenge2", 5:"dailyChallenge5", 10:"dailyChallenge10",
                20:"dailyChallenge20", 30:"dailyChallenge30" };
  return map[n] || null;
};
...
const getSpinnerKey = (n) => {
  const map = { 3:"spinner3", 6:"spinner6", 10:"spinner10",
                20:"spinner20", 40:"spinner40", 60:"spinner60", 100:"spinner100" };
  return map[n] || null;
};

const getWithdrawalKey = (n) => {
  const map = { 1:"withdrawal1st", 2:"withdrawal2nd", 3:"withdrawal3rd",
                4:"withdrawal4th", 6:"withdrawal6th", 10:"withdrawal10th" };
  return map[n] || null;
};
...
const getGamePurchaseCountKey = (n) => {
  ...
  const map = { 1:"gamePurchase1st", 2:"gamePurchase2nd", 5:"gamePurchase5th",
                10:"gamePurchase10th", 20:"gamePurchase20th", 40:"gamePurchase40th" };
  return map[n] || null;
};
...
export const onGameDownload = (totalDownloads) => {
  ...
  if (!key) return;
  fireEvent(SDK_TOKENS[key], S2S[key], {}, { metadata: { downloadCount: totalDownloads } });
};
...
export const onSurveyComplete = (totalSurveys, surveyId) => {
  ...
  if (!key) return;
  fireEvent(SDK_TOKENS[key], S2S[key], {}, { metadata: { surveyId, count: totalSurveys } });
};
...
export const onNonGamingOfferComplete = (totalOffers) => {
  const map = { 1:"nonGamingOffer1st", 3:"nonGamingOffer3rd", 5:"nonGamingOffer5th",
                10:"nonGamingOffer10th", 20:"nonGamingOffer20th", 30:"nonGamingOffer30th" };
  const key = map[totalOffers];
  if (!key) return;
  fireEvent(SDK_TOKENS[key], S2S[key], {}, { metadata: { offerCount: totalOffers } });
};
...
export const onDailyChallengeComplete = (totalChallenges, challengeId) => {
  ...
  if (!key) return;
  fireEvent(SDK_TOKENS[key], S2S[key], {}, { metadata: { challengeId, count: totalChallenges } });
};
...
export const onRaceGameComplete = (racePosition) => {
  const map = { 1:"race1st", 2:"race2nd", 3:"race3rd", 4:"race4th", 5:"race5th" };
  const key = map[racePosition];
  if (!key) return;
  fireEvent(SDK_TOKENS[key], S2S[key], {}, { metadata: { position: racePosition } });
};
...
export const onCashWithdrawal = (withdrawalCount) => {
  ...
  if (!key) return;
  fireEvent(SDK_TOKENS[key], S2S[key], {}, { metadata: { count: withdrawalCount } });
};
...
export const onInGamePurchase = (purchaseCount, revenue, productId) => {
  ...
  if (countKey) {
    fireEvent(SDK_TOKENS[countKey], S2S[countKey], {}, { metadata: { purchaseCount, productId } });
  }

  const revenueKey = getPurchaseRevenueKey(revenue);
  if (revenueKey) {
    fireEvent(
      SDK_TOKENS[revenueKey], S2S[revenueKey],
  ...
  fireEvent(
    SDK_TOKENS.inGamePurchaseRevenue, S2S.inGamePurchaseRevenue,
    { revenue, currency: "USD" },
    { revenue, currency: "USD" }
  );
};
...
export const onXPLevelReached = (level) => {
  ...
  if (level === "senior") fireEvent(SDK_TOKENS.reachSeniorXP, S2S.reachSeniorXP);
};
...
export const fetchUserRetention = async (authToken) => {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[Adjust Retention] ❌ GET /v2/adjust/retention returned ${res.status}`);
      return null;
    }
    const json = await res.json();
    const data = json?.data;
    console.log(`[Adjust Retention] ✅ Fetched — isFirstOpen: ${data?.isFirstOpen} | daysSinceFirstOpen: ${data?.daysSinceFirstOpen} | firedMilestones: [${data?.firedMilestones?.join(",")}]`);
    return data || null;
  } catch (err) {
...
export const markRetentionMilestone = (authToken, day) => {
  ...
  fetch(`${BASE_URL}/api/v2/adjust/retention/milestone`, {
    ...
    .then((res) => {
      if (res.ok) console.log(`[Adjust Retention] ✅ Milestone Day ${day} recorded on backend`);
      else        console.warn(`[Adjust Retention] ❌ Milestone Day ${day} failed — status: ${res.status}`);
    })
    .catch((err) => console.warn(`[Adjust Retention] ❌ Milestone Day ${day} network error:`, err?.message));
};
...
export const onAppOpenWithRetention = (retentionData, authToken) => {
  const { isFirstOpen, daysSinceFirstOpen, firedMilestones = [] } = retentionData;

  console.log(`[Adjust] 📱 onAppOpenWithRetention — isFirstOpen: ${isFirstOpen} | days: ${daysSinceFirstOpen} | firedMilestones: [${firedMilestones.join(",")}]`);
  ...
  if (isFirstOpen) {
    console.log("[Adjust] 🎉 First open for this user account — firing install event!");
    fireEvent(SDK_TOKENS.appInstall, S2S.appInstall);
  }
  ...
  retentionDays.forEach((day) => {
    if (!firedMilestones.includes(day) && daysSinceFirstOpen >= day) {
      console.log(`[Adjust] 🏆 Retention milestone — Day ${day} firing (days since first open: ${daysSinceFirstOpen})`);
