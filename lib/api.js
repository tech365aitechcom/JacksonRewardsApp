...
const apiRequest = async (
  ...
  // Android native app — required by backend for platform identification
  if (
    typeof window !== "undefined" &&
    ...
    console.log(
      "[PlayIntegrity] x-platform: android header added for",
      endpoint,
    );
  }
  ...
  if (token) {
    ...
    // Google Play Integrity: attach token for protected endpoints (Android only).
    // Fresh nonce fetched from backend per request — never reused (single-use per PDF spec).
    if (
      typeof window !== "undefined" &&
      window.Capacitor?.isNativePlatform?.() === true &&
      window.Capacitor?.getPlatform?.() === "android"
      ...
      try {
        console.log(
          "[PlayIntegrity] 🔐 Requesting integrity token for",
          endpoint,
        );
        ...
        if (integrityToken) {
          ...
          console.log(
            "[PlayIntegrity] ✅ x-integrity-token attached for",
            endpoint,
          );
        } else {
          console.warn(
            "[PlayIntegrity] ⚠️ No integrity token returned for",
            endpoint,
          );
        }
      } catch (e) {
