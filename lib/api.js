...
const BASE_URL = 'https://rewardsuatapi.hireagent.co' || 'http://localhost:5000'
...
// Endpoints that require Google Play Integrity token (X-Integrity-Token) per backend Postman collection
const ENDPOINTS_REQUIRING_INTEGRITY = [
  '/api/payout/create', // High security – withdrawal
  '/api/payouts/create', // High security – Tremendous payout
  '/api/game/earn', // Medium – transfer game earnings
  '/api/daily-challenge/complete', // Medium – complete challenge
  '/api/daily-rewards/claim', // Medium – claim daily reward
  '/api/daily-rewards/recover', // Medium – recover missed day
  '/api/streak/claim-reward', // Medium – claim streak milestone
]
  ...
  try {
    ...
    if (endpoint.startsWith('/api/fraud-prevention')) {
      if (endpoint.includes('/session/authenticate')) {
        ...
        const pathId = endpoint.split('/session/')[1]?.split('/')[0] || endpoint
        console.log(
...
export const getFundingSources = (token, queryParams = {}) => {
  ...
  return apiRequest(`/api/payout/funding-sources?${params}`, 'GET', null, token)
}
...
export const getGamesBySection = async ({
  ...
  if (userToUse) {
    ...
    const userGender =
      userToUse.gender || userToUse.profile?.gender || userToUse.details?.gender
...
export const getBitlabsAIDownloadedGames = (userId, token) => {
  const endpoint = `/api/bitlabs/user-history/${userId}/1671770`
  return apiRequest(endpoint, 'GET', null, token)
}
...
export const APPLOVIN_CONFIG = {
  AD_UNIT_ID_ANDROID_REWARDED: '6e87990ef5f63cce',
  GOOGLE_ADMOB_APP_ID: 'ca-app-pub-2800391972465887~5310386906',
}
 ...
 */
export const getAppLovinHealth = () => apiRequest('/api/applovin/health', 'GET')
...
export const trackAppLovinAdDisplay = (adRecordId, token) =>
  apiRequest('/api/applovin/rewarded-ad/display', 'POST', { adRecordId }, token)
