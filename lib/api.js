    ...
    if (
      ...
      try {
        ...
        if (integrityToken) {
          ...
          console.warn(
            "[PlayIntegrity]  No integrity token returned for",
            endpoint,
          );
        }
      } catch (e) {
    ...
    if (endpoint.includes("/biometric")) {
      ...
      if (endpoint.includes("/biometric-login")) {
        ...
        if (
          ...
          if (extractedUser && !extractedUser._id) {
            console.warn("[API] Biometric login user object missing _id:", {
...
export const transferGameEarnings = async (earningData, token) => {
  ...
  const payload = {
    ...
    // Task IDs for per-task deduplication (prevent claiming same task twice)
