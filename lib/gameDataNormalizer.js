...
export const getSdkProvider = (game) => {
  ...
  let provider = rawData.sdkProvider || rawData.provider || game?.sdkProvider || game?.provider;
  if (provider) {
...
export const normalizeGameImages = (game) => {
  ...
  if (provider === 'bitlab') {
    ...
    const getAllImageValues = () => {
      ...
      const sizeKeys = ['275x275', '400x400', '580x580', '630x315', '600x300', '600x200', '800x400', '1024x512'];
...
export const normalizeGameTitle = (game) => {
  ...
  if (provider === 'bitlab') {
    return rawData.product_name || rawData.anchor || rawData.title || game?.title || game?.details?.name || 'Unknown Game';
  } else {
    return rawData.title || game?.title || game?.details?.name || 'Unknown Game';
  }
};
...
export const normalizeGameDescription = (game) => {
  ...
  if (provider === 'bitlab') {
    return rawData.description || game?.description || game?.details?.description || '';
  } else {
    return rawData.description || game?.description || game?.details?.description || '';
  }
};
...
function getEventsFromGame(game) {
  ...
  const candidates = [
    ...
    game?.offer?.events,
    game?.raw?.events,
    Array.isArray(game?.data?.available?.[0]?.events) ? game.data.available[0].events : null,
  ];
...
export const normalizeGameGoals = (game) => {
  ...
  if (provider === 'bitlab') {
      ...
      return events.map((event, index) => {
        const promisedPoints =
          event.promised_points != null
            ...
            : (event.points != null ? String(event.points) : event.payout != null ? String(event.payout) : '0');
        let daysLeft = null;
        if (event.expires_at && event.expires_at > 0) {
          const expiresAtMs = event.expires_at * 1000;
          daysLeft = Math.ceil((expiresAtMs - Date.now()) / (1000 * 60 * 60 * 24));
        ...
        const isCompleted = event.status === 'completed' || event.status === 'success' || event.completed === true;
        const isFailed = event.status === 'failed' || event.status === 'expired' || event.completed === false;
        // Event number from API (id / event_number) for XP multiplier and unlock order; fallback to 1-based index
        const eventNumber = event.event_number ?? (typeof event.id === 'number' ? event.id : null) ?? (index + 1);
        return {
          ...
          goal_type: event.type_id === 2 ? 'linear' : event.type_id === 4 ? 'non-linear' : 'linear',
          section: event.type_id === 4 ? 'turbo' : event.type_id === 1 ? 'install' : 'linear',
          position: index + 1,
          ...
          status: event.status || (isCompleted ? 'completed' : isFailed ? 'failed' : 'viewed'),
          timestamp: event.timestamp,
          completed_datetime: isCompleted && event.timestamp ? event.timestamp : null,
    ...
    return goals.map((goal, index) => {
      ...
      let amount = goal.promised_points ?? goal.points ?? goal.amount ?? goal.xp ?? goal.reward ?? goal.value ?? goal.reward_points ?? goal.approved_cpa ?? goal.pending_cpa;
      if (amount == null || amount === '') {
        const arr = rawData.promised_points ?? game?.promised_points ?? rawData.reward_points ?? game?.reward_points;
        if (Array.isArray(arr) && arr[index] != null) amount = arr[index];
      }
      if (amount == null || amount === '') {
        const ev = eventsByIndex[index];
        if (ev && (ev.promised_points != null || ev.points != null)) amount = ev.promised_points ?? ev.points;
      }
      const promisedPoints = amount != null && amount !== '' ? String(amount) : '0';
      const name = goal.name ?? goal.title ?? goal.text ?? '';
      const isCompleted = goal.completed === true || goal.status === 'completed' || goal.status === 'success';
      const isFailed = goal.failed === true || goal.status === 'failed' || goal.status === 'expired';
      const numId = typeof goal.id === 'number' ? goal.id : null;
      const eventMatch = typeof goal.id === 'string' ? goal.id.match(/^event-(\d+)$/) : null;
      const parsedEventN = eventMatch ? parseInt(eventMatch[1], 10) + 1 : null;
      const eventNumber = goal.event_number ?? numId ?? parsedEventN ?? (index + 1);
      return {
        ...
        goal_type: goal.goal_type ?? (goal.type === 'flat' ? 'linear' : 'linear'),
        ...
        status: goal.status ?? (isCompleted ? 'completed' : isFailed ? 'failed' : 'viewed'),
        timestamp: goal.timestamp,
        completed_datetime: goal.completed_datetime ?? (isCompleted && goal.timestamp ? goal.timestamp : null),
...
export const getTotalPromisedPoints = (game) => {
  ...
  if (provider === 'bitlab') {
    ...
    const totalPointsFromApi = rawData.total_points != null ? parseFloat(rawData.total_points) : NaN;
    const totalCoins =
      (rewardsCoins != null && Number(rewardsCoins) > 0)
        ...
        : Number.isFinite(totalPointsFromApi) && totalPointsFromApi > 0
          ...
          : events.reduce(
              (acc, e) =>
                acc + (parseFloat(e.promised_points ?? e.points ?? e.payout) || 0),
              0
            ) ||
            (rawData.goals || game?.goals || []).reduce(
              (acc, g) => acc + (parseFloat(g.promised_points ?? g.amount ?? g.points) || 0),
              0
            );

    // Total XP = sum of baseXP * multiplier^index for each task (geometric series)
    const xpConfig = rawData.xpRewardConfig || game?.xpRewardConfig || { baseXP: 1, multiplier: 1 };
    ...
    const finalXP = (rewardsXP != null && Number(rewardsXP) > 0) ? Number(rewardsXP) : totalXP;
    return {
  ...
  const rewardsCoins = game?.rewards?.coins ?? game?.rewards?.gold;
  const goals = rawData.goals || game?.goals || [];
  const sumCoins = goals.reduce((acc, g) => acc + (parseFloat(g.amount) || 0), 0);
  const totalCoins = (rewardsCoins != null && Number(rewardsCoins) >= 0)
    ? Number(rewardsCoins)
    : sumCoins;
  ...
  let totalXP;
  if (n === 0) {
    totalXP = (game?.rewards?.xp != null && Number(game.rewards.xp) >= 0) ? Number(game.rewards.xp) : 0;
  ...
  const rewardsXP = game?.rewards?.xp;
  const finalXP = (rewardsXP != null && Number(rewardsXP) > 0) ? Number(rewardsXP) : Math.floor(totalXP);
  return {
    totalCoins: Math.round(Number(totalCoins) * 100) / 100,
    totalXP: Math.round(Number(finalXP))
  };
...
export const normalizeGameUrl = (game) => {
  ...
  if (provider === 'bitlab') {
    ...
    console.log('🔗 Bitlab URL extraction:', {
      ...
      allUrlFields: Object.keys(rawData).filter(key =>
  ...
  if (provider === 'bitlab') {
    ...
    const totalAmount = events
      ...
      .reduce((sum, event) => {
        ...
        const points = parseFloat(event.promised_points || event.points || 0) / 100;
