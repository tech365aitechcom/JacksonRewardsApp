...
const getAppLovinPlugin = () => {
  ...
  if (window.applovin) {
    ...
    if (typeof window.applovin.initialize === "function") {
      ...
      console.warn("[AppLovin]  window.applovin exists but initialize is not a function");
    }
  }
...
class AppLovinMAXPlugin {
      ...
      if (isNative) {
        ...
        if (!pluginInfo && isNative) {
          ...
          try {
            ...
            console.warn("[AppLovin]  Failed to load plugin script:", error);
          }
          ...
          // Final check
          if (!pluginInfo) {
            console.warn("[AppLovin]  Plugin still not found after all attempts");
        ...
        if (pluginInfo) {
          ...
          console.warn("[AppLovin]  Native plugin not available");
  ...
  setupCordovaListeners() {
    ...
    window.addEventListener("OnRewardedAdReceivedRewardEvent", (event) => {
      ...
      if (this.completionFired) {
        console.warn("[AppLovin]  Completion event already fired, ignoring duplicate");
        return;
      }
  ...
  setupCapacitorListeners() {
    ...
    this.nativePlugin.addListener("rewardedAdReceivedReward", (result) => {
      // Prevent multiple completion events from firing
      if (this.completionFired) {
        console.warn("[AppLovin]  Capacitor completion event already fired, ignoring duplicate");
        return;
      }
  ...
  showMediationDebugger() {
    ...
    if (pluginInfo?.plugin?.showMediationDebugger) {
      ...
      console.warn("[AppLovin]  Mediation Debugger not available — must run on real device after build");
    }
  }
