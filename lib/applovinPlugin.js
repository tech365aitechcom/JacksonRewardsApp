 ...
 *
 * Ad Unit IDs:
 * - Android Rewarded: 6e87990ef5f63cce
 * - Google AdMob App ID: ca-app-pub-2800391972465887~5310386906
 */
      ...
      await new Promise((resolve, reject) => {
        script.onload = () => {
          ...
          if (window.applovin || window.AppLovinMAX) {
            ...
            reject(
              new Error(
                "Script loaded but AppLovin global not found (expected window.applovin)",
              ),
            );
          }
        };
...
// Ad Unit IDs provided by client
export const APPLOVIN_CONFIG = {
  SDK_KEY: "", // Will be fetched from backend via /applovin/config API
  AD_UNIT_ID: {
    // Android Rewarded Ad Unit ID
    ANDROID_REWARDED: "6e87990ef5f63cce",
    // iOS Rewarded Ad Unit ID
    IOS_REWARDED: "b3bf29fc650db20d",
  },
  GOOGLE_ADMOB_APP_ID: {
    // Android Google AdMob App ID
    ANDROID: "ca-app-pub-2800391972465887~5310386906",
    // iOS Google AdMob App ID
    IOS: "ca-app-pub-2800391972465887~6894931313",
  },
};
...
const getAppLovinPlugin = () => {
  ...
  if (window.applovin) {
    ...
    console.log(
      "[AppLovin] 📊 applovin keys:",
      Object.keys(window.applovin || {}),
    );

    if (typeof window.applovin.initialize === "function") {
      console.log(
        "[AppLovin] ✅ Found cordova-plugin-applovin-max (applovin.initialize exists)",
      );
      return { type: "cordova", plugin: window.applovin };
    } else {
      console.warn(
        "[AppLovin] ⚠️ window.applovin exists but initialize is not a function",
      );
    }
  }
...
class AppLovinMAXPlugin {
  ...
  async initialize(config = {}) {
    ...
    const admobAppId =
      platform === "ios"
        ? APPLOVIN_CONFIG.GOOGLE_ADMOB_APP_ID.IOS
        : APPLOVIN_CONFIG.GOOGLE_ADMOB_APP_ID.ANDROID;
    console.log(
      "[AdMob/UMP] Google AdMob App ID (mob id) for",
      platform,
      ":",
      admobAppId,
    );
    ...
    try {
      ...
      if (isNative) {
        ...
        console.log(
          "[AppLovin] 📱 Native platform detected, initializing native SDK...",
        );
        console.log("[AppLovin] 🔍 Checking for native plugin...");
        console.log(
          "[AppLovin] 🌐 Window object available:",
          typeof window !== "undefined",
        );
        console.log(
          "[AppLovin] 🔍 Checking window.applovin:",
          typeof window?.applovin,
        );
        console.log(
          "[AppLovin] 🔍 Checking window.AppLovinMAX (legacy):",
          typeof window?.AppLovinMAX,
        );
        console.log(
          "[AppLovin] 🔍 Checking window.Capacitor?.Plugins?.AppLovinMAX:",
          typeof window?.Capacitor?.Plugins?.AppLovinMAX,
        );
        console.log(
          "[AppLovin] 🔍 Checking window.cordova?.plugins?.AppLovinMax:",
          typeof window?.cordova?.plugins?.AppLovinMax,
        );

        // Wait for plugin to be available (Cordova plugins load after deviceready)
        let pluginInfo = getAppLovinPlugin();

        // If plugin not found and we're on native, try to load it and retry
        if (!pluginInfo && isNative) {
          console.log(
            "[AppLovin] ⏳ Plugin not found immediately, attempting to load...",
          );
          console.log("[AppLovin] 📊 Current state:", {
            readyState:
              typeof document !== "undefined" ? document.readyState : "N/A",
            hasCapacitor: typeof window?.Capacitor !== "undefined",
            hasCordova: typeof window?.cordova !== "undefined",
          });
          ...
          if (!pluginInfo) {
            ...
            for (let attempt = 0; attempt < 5; attempt++) {
              ...
              if (pluginInfo) {
                console.log(
                  `[AppLovin] ✅ Plugin found after ${attempt + 1} retry(ies)!`,
                );
                break;
              } else {
                console.log(
                  `[AppLovin] 🔄 Retry ${attempt + 1}/5: Plugin still not found...`,
                );
              }
            }
          }
          ...
          if (!pluginInfo) {
            console.warn(
              "[AppLovin] ⚠️ Plugin still not found after all attempts",
            );
            console.warn("[AppLovin] 💡 Solutions:");
            console.warn(
              "[AppLovin]   1. Copy AppLovinMax.js to public/plugins/ folder",
            );
            console.warn("[AppLovin]   2. Add script tag to index.html");
            console.warn(
              "[AppLovin]   3. Check capacitor.config.ts for plugin configuration",
            );
            console.warn(
              "[AppLovin] 📝 See APPLOVIN_PLUGIN_NOT_LOADING_FIX.md for details",
            );
          }
        }
        ...
        if (pluginInfo) {
          ...
          try {
            if (pluginInfo.type === "cordova") {
              ...
              console.log(
                "[AppLovin] Initializing Cordova plugin with SDK key...",
              );
              await new Promise((resolve, reject) => {
                try {
                  pluginInfo.plugin.initialize(this.sdkKey, (config) => {
                    ...
                    try {
                      if (
                        ...
                        console.log(
                          "[AppLovin] ✅ Verbose logging enabled (dev mode)",
                        );
                      }
                    } catch (e) {
                      console.log(
                        "[AppLovin] ⚠️ Could not enable verbose logging:",
                        e?.message || e,
                      );
                    }
            ...
            console.error(
              "[AppLovin] ❌ Native SDK initialization failed:",
              error,
            );
          ...
          console.warn(
            "[AppLovin] To use real ads, install: npm install cordova-plugin-applovin-max",
          );
        ...
        console.log(
          "[AppLovin] Running in web mode - using mock implementation",
        );
      }
  ...
  setupCordovaListeners() {
    ...
    window.addEventListener("OnRewardedAdLoadFailedEvent", (event) => {
      ...
      this.isAdLoaded = false;
      this.adFailListeners.forEach((listener) =>
        listener({
          type: "load",
    ...
    window.addEventListener("OnRewardedAdReceivedRewardEvent", (event) => {
      ...
      if (this.completionFired) {
        console.warn(
          "[AppLovin] ⚠️ Completion event already fired, ignoring duplicate",
        );
        return;
      }
      ...
      const result = {
        ...
        adInfo: {
          ...
          adNetwork: (rewardInfo.networkName || "applovin")
            .toLowerCase()
            .replace(/\s+/g, "_"),
      ...
      console.log(
        "[AppLovin] 📢 Notifying",
        this.adCompleteListeners.length,
  ...
  setupCapacitorListeners() {
    ...
    this.nativePlugin.addListener("rewardedAdReceivedReward", (result) => {
      ...
      if (this.completionFired) {
        console.warn(
          "[AppLovin] ⚠️ Capacitor completion event already fired, ignoring duplicate",
        );
        return;
      }
      ...
      this.adCompleteListeners.forEach((listener) => {
        try {
          ...
          console.error(
            "[AppLovin] ❌ Error in Capacitor completion listener:",
            err,
          );
        }
      });
    ...
    if (isNative && this.nativePlugin) {
        ...
        const attemptLoad = () => {
          const loadTimeout = setTimeout(() => {
            if (retryCount < maxRetries) {
              retryCount++;
              console.log(
                `[AppLovin] ⏳ Ad load timeout, retrying (${retryCount}/${maxRetries})...`,
              );
              ...
              reject(
                new Error(
                  "Ad load timeout after retries. Please check your network connection.",
                ),
              );
            }
          }, timeoutMs);
          ...
          const onLoaded = (adInfo) => {
            ...
            this.removeFailListener(onFailed);
            console.log(
              `[AppLovin] ✅ Ad loaded successfully${retryCount > 0 ? ` (after ${retryCount} retry)` : ""}`,
            );
            resolve(adInfo);
          };
          ...
          const onFailed = (error) => {
            clearTimeout(loadTimeout);
            const errorMsg =
              error?.error || error?.message || "Failed to load ad";
            ...
            if (isNetworkError && retryCount < maxRetries) {
              retryCount++;
              console.log(
                `[AppLovin] ⚠️ Network error detected, retrying (${retryCount}/${maxRetries}):`,
                errorMsg,
              );
              this.removeLoadListener(onLoaded);
              this.removeFailListener(onFailed);
          ...
          if (this.pluginType === "cordova") {
            ...
            console.log(
              "[AppLovin] Loading rewarded ad with unit ID:",
              this.adUnitId,
            );
            ...
            this.nativePlugin
              .loadRewardedAd({ adUnitId: this.adUnitId })
              .then((adInfo) => {
                onLoaded(adInfo);
              })
              .catch((error) => {
      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          ...
          this.adLoadListeners.forEach((listener) =>
            listener(this.currentAdInfo),
          );
  ...
  isAdReady() {
    if (isNative && this.nativePlugin) {
      if (this.pluginType === "cordova") {
        ...
        try {
          const result = this.nativePlugin.isRewardedAdReady({
            adUnitId: this.adUnitId,
          });
    ...
    if (isNative && this.nativePlugin) {
      ...
      return new Promise((resolve, reject) => {
        ...
        const displayTimeout = setTimeout(() => {
          if (!timeoutCleared) {
            console.warn(
              "[AppLovin] Display timeout fired - ad completion event may not have been received",
            );
            reject(
              new Error(
                "Ad display timeout - please check your internet connection and try again",
              ),
            );
          }
        ...
        if (this.pluginType === "cordova") {
          ...
          console.log(
            "[AppLovin] Showing rewarded ad with unit ID:",
            this.adUnitId,
            "placement:",
            placement,
          );
          ...
          // Capacitor plugin uses Promise-based API
          this.nativePlugin
            .showRewardedAd({ adUnitId: this.adUnitId, placement })
            .then((result) => {
              onComplete(result);
            })
            .catch((error) => {
      ...
      this.adDisplayListeners.forEach((listener) =>
        listener(this.currentAdInfo),
      );
      ...
      return new Promise((resolve, reject) => {
        ...
        console.log(
          `[AppLovin Mock] Ad playing for ${adDuration / 1000} seconds...`,
        );
          ...
          if (success) {
            ...
            const result = {
              ...
              adInfo: {
                ...
                ...(this.currentAdInfo?.revenue &&
                typeof this.currentAdInfo.revenue === "number"
                  ? {
                      revenue: {
                        amount: this.currentAdInfo.revenue,
                        currency: "USD",
                      },
                    }
                  : {}),
  ...
  removeDisplayListener(listener) {
    this.adDisplayListeners = this.adDisplayListeners.filter(
      (l) => l !== listener,
    );
  }
  ...
  removeCompleteListener(listener) {
    this.adCompleteListeners = this.adCompleteListeners.filter(
      (l) => l !== listener,
    );
  ...
  showMediationDebugger() {
    const pluginInfo = getAppLovinPlugin();
    if (pluginInfo?.plugin?.showMediationDebugger) {
      console.log("[AppLovin] 🔍 Opening Mediation Debugger...");
      pluginInfo.plugin.showMediationDebugger();
    } else {
      console.warn(
        "[AppLovin] ⚠️ Mediation Debugger not available — must run on real device after build",
      );
    }
  }
