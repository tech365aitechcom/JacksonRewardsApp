...
const KEYS = ["survey", "gameDownload", "withdrawal", "spin", "challenge", "nongameOffer"];
...
export const initFromServer = async (authToken) => {
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[AdjustCounters] /v2/adjust/counts returned ${res.status} — using 0 as base`);
      return;
    }
    ...
    console.warn("[AdjustCounters] Seed failed — using 0 as base:", err?.message);
  }
};
...
export const resetCounters = () => {
  ...
  _initialized = false;
  console.log("[AdjustCounters] 🔄 Reset — next login will re-seed from server");
};
