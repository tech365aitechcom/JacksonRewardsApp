  ...
  async (
    ...
    try {
      ...
      if (offerType === "cashback_shopping" || offerType === "all") {
        ...
        // Process cashback offers - show up to 2 (prefer categorized so we get full list)
        if (cashbackResponse.success && cashbackResponse.data) {
          const cashbackList = cashbackResponse.data.categorized?.cashback ?? cashbackResponse.data.offers ?? [];
          offers = [...offers, ...(Array.isArray(cashbackList) ? cashbackList.slice(0, 2) : [])];
        }

        // Process shopping offers - show up to 1 (prefer categorized so we get full list)
        if (shoppingResponse.success && shoppingResponse.data) {
          const shoppingList = shoppingResponse.data.categorized?.shopping ?? shoppingResponse.data.offers ?? [];
          offers = [...offers, ...(Array.isArray(shoppingList) ? shoppingList.slice(0, 1) : [])];
        }
