...
export const fetchBitlabsDownloadedGames = createAsyncThunk(
  ...
  async ({ userId, token }, { rejectWithValue }) => {
    try {
      ...
      console.log('Bitlabs Debug - Fetching:', { userId, offerId, hasToken: !!token });
...
export const fetchBitlabsAIDownloadedGames = createAsyncThunk(
  ...
  async ({ userId, token }, { rejectWithValue }) => {
    try {
      console.log('Bitlabs AI Debug - Fetching:', { userId, hasToken: !!token });
    ...
    builder
      ...
      .addCase(fetchBitlabsDownloadedGames.fulfilled, (state, action) => {
        ...
        if (action.payload.data) {
          ...
          state.bitlabsDownloadedGamesError = action.payload.error.message || "API error";
          return;
        }
        ...
        const taskProgressionRule = action.payload.data?.taskProgressionRule || null;
        ...
        const mappedGames = bitlabsGames.map((game) => {
          ...
          const completedEvents = game.events?.filter(event => event.status === "completed") || [];
          const totalPayableEvents = game.events?.filter(event => event.payable) || [];
          ...
          const processEventWithProgression = (event, index) => {
            ...
            if (hasProgressionRule && firstBatchSize > 0) {
              if (index < firstBatchSize) {
                ...
                const batch0Events = game.events?.slice(0, firstBatchSize) || [];
                const batch0Completed = batch0Events.every(e => e.status === "completed");
                if (!batch0Completed) {
                  allPreviousBatchesCompleted = false;
                }
                
                // Check all batches before current batch
                for (let b = 1; b < batchNumber && allPreviousBatchesCompleted; b++) {
                  const batchStart = firstBatchSize + (b - 1) * nextBatchSize;
                  const batchEnd = Math.min(batchStart + nextBatchSize, game.events?.length || 0);
                  const batchEvents = game.events?.slice(batchStart, batchEnd) || [];
                  const batchCompleted = batchEvents.every(e => e.status === "completed");
            ...
            return {
              ...
              progression: {
                isUnlocked,
                isLocked,
                unlockReason: isLocked ? "Complete previous tasks to unlock" : (event.progression?.unlockReason || "No progression rules applied"),
                batchNumber,
          ...
          const goals = game.events?.map((event, index) => {
            const processed = processEventWithProgression(event, index);
            console.log(`🔒 [Bitlabs] Event ${index} "${event.name}": batch=${processed.progression.batchNumber}, locked=${processed.progression.isLocked}, unlocked=${processed.progression.isUnlocked}`);
            return processed;
          }) || [];
          
          // Calculate completed unlocked tasks count for progression
          const completedUnlockedTasks = goals.filter(g => g.completed && !g.progression.isLocked).length;
          ...
          const calculateCanUnlockNextTasks = () => {
            ...
            const tasksAfterFirstBatch = completedUnlockedTasks - firstBatchSize;
            const currentBatch = Math.floor(tasksAfterFirstBatch / nextBatchSize);
          ...
          return {
            ...
            categories: game.categories?.map(c => typeof c === 'string' ? { name: c } : c) ,
            goals,
        ...
        if (state.currentGameDetails && state.currentGameDetails.source === "bitlabs") {
          const activeGame = mappedGames.find(g => g.id === state.currentGameDetails.id);
          if (activeGame) {
      ...
      .addCase(fetchBitlabsAIDownloadedGames.fulfilled, (state, action) => {
        ...
        const mappedAIGames = bitlabsAIGames.map((game) => {
          ...
          const earnedAmount = events
            ...
            .reduce((total, event) => total + (parseFloat(event.promised_points) || 0), 0);
          ...
          const goals = events.map((event, index) => ({
            ...
            amount: event.promised_points != null ? String(event.promised_points) : "0",
            completed: event.status === "completed",
          ...
          return {
            ...
            categories: game.categories?.map(c => typeof c === 'string' ? { name: c } : c) || [{ name: "Game" }],
        ...
        if (state.currentGameDetails && state.currentGameDetails.source === "bitlabs_ai") {
          const activeGame = mappedAIGames.find(g => g.id === state.currentGameDetails.id);
          if (activeGame) {
            state.currentGameDetails = activeGame;
            state.gameDetailsStatus = "succeeded";
          }
