    ...
    builder
      ...
      .addCase(fetchUserData.fulfilled, (state, action) => {
        ...
        if (!fromCache) {
          ...
          // Debug: log Besitos in_progress games reward data
          if (userDataPayload.in_progress?.length > 0) {
            console.log('💰 [Besitos-DL] In-progress games:', userDataPayload.in_progress.map(g => ({
              id: g.id, title: g.title, goalsCount: g.goals?.length,
              goals: g.goals?.map(goal => ({ name: goal.text || goal.name, amount: goal.amount, coinReward: goal.coinReward, days_left: goal.days_left, completed: goal.completed, failed: goal.failed }))
            })));
          }
      ...
      .addCase(fetchGamesBySection.fulfilled, (state, action) => {
        ...
        // Never overwrite existing games with empty (prevents list disappearing on late/empty response)
        if (!fromCache) {
          if (hasNewGames) {
          ...
          // ALWAYS update timestamp on a real API response — even when the response is empty
          // and existing games are kept. Without this, hasFreshCache stays false and the
          // component's useEffect dispatches again immediately → infinite loop.
          ...
          // Always set succeeded when we have section data so UI shows content (including when request was background)
      ...
      .addCase(fetchBitlabsDownloadedGames.fulfilled, (state, action) => {
        ...
        const mappedGames = bitlabsGames.map((game) => {
          ...
          const processEventWithProgression = (event, index) => {
            ...
            if (hasProgressionRule && firstBatchSize > 0) {
              if (index < firstBatchSize) {
                ...
                isLocked = false;
                isUnlocked = true;
              } else if (!nextBatchSize || nextBatchSize === 0) {
                // nextBatchSize is 0 or null: all remaining tasks unlock freely after first batch
                batchNumber = null;
                isLocked = false;
                isUnlocked = true;
              } else {
            ...
            console.log('🔍 [BitLabs-DL] Processing event:', { uuid: event.uuid, name: event.name, promised_points: event.promised_points, coinReward: event.coinReward, payable: event.payable, status: event.status, type_id: event.type_id });
            return {
              id: event.uuid,
              name: event.name,
          ...
          const calculateCanUnlockNextTasks = () => {
            ...
            // If nextBatchSize is 0 or null, all tasks unlock after first batch
          ...
          console.log('💰 [BitLabs-DL] Mapped game:', { id: game.offer_id || game.id, name: game.product_name, goalsCount: goals.length, earnedAmount, totalPoints: game.total_points, goals: goals.map(g => ({ id: g.id, name: g.name, amount: g.amount, coinReward: g.coinReward, payable: g.payable, completed: g.completed })) });
      ...
      .addCase(fetchBitlabsAIDownloadedGames.fulfilled, (state, action) => {
        ...
        const mappedAIGames = bitlabsAIGames.map((game) => {
          ...
          console.log('🔍 [BitLabs-AI-DL] Raw events:', events.map(e => ({ uuid: e.uuid, name: e.name, promised_points: e.promised_points, coinReward: e.coinReward, payable: e.payable, status: e.status })));
          ...
          console.log('💰 [BitLabs-AI-DL] Goals mapped:', { goalsCount: goals.length, earnedAmount, goals: goals.map(g => ({ name: g.name, amount: g.amount, coinReward: g.coinReward, payable: g.payable, completed: g.completed })) });
          ...
          console.log('💰 [BitLabs-AI-DL] Mapped game:', { id: game.id || game.offer_id, name: game.product_name, goalsCount: goals.length, earnedAmount, totalPoints: game.total_points });
