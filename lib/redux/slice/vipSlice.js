...
export const confirmGooglePlayPayment = createAsyncThunk(
  ...
  async ({ subscriptionId, purchaseToken, productId, orderId, token }, { rejectWithValue }) => {
    try {
      ...
      const response = await verifyGooglePlayPurchase(
        {
          ...
          orderId,
          metadata: {
            subscriptionId, // Include subscription ID in metadata for backend tracking
            platform: "android",
            appVersion: "1.0.0"
          }
        },
      ...
      if (response.success === false) {
        throw new Error(
          response.error || response.message || "Google Play payment verification failed"
        );
      }

      return response.data || response;
    } catch (error) {
      console.error("❌ [confirmGooglePlayPayment] Error:", error);
