...
import { getProductId } from "@/lib/googlePlayBilling";
...
// Confirm a completed Google Play purchase with the backend
export const confirmGooglePlayPayment = createAsyncThunk(
  ...
  async ({ subscriptionId, purchaseToken, productId, orderId, token }, { rejectWithValue }) => {
    try {
      ...
      const response = await confirmGooglePlayPurchase(
        { subscriptionId, purchaseToken, productId, orderId },
        token
      );
      ...
      if (response.success === false) {
        throw new Error(
          response.error || response.message || "Google Play payment confirmation failed"
        );
      }

      return response.data || response;
    } catch (error) {
      return rejectWithValue(error.message || "Google Play payment confirmation failed");
    }
  }
);
