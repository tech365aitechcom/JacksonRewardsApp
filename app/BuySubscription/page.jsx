...
import {
  ...
  fetchActiveGooglePlaySubscription,
...
const StarIcon = ({ className, color }) => (
  ...
  >
    <path
      d='M9.5 0L11.6329 7.36714H19L13.1836 11.9214L15.3164 19L9.5 14.4457L3.68359 19L5.81641 11.9214L0 7.36714H7.36719L9.5 0Z'
      fill={color}
    />
  </svg>
)
const SparkleIcon = ({ className, color }) => (
  ...
  >
    <path
      d='M9.5 0L12.5858 6.41421L19 9.5L12.5858 12.5858L9.5 19L6.41421 12.5858L0 9.5L6.41421 6.41421L9.5 0Z'
      fill={color}
    />
  </svg>
)
const SparkleSmallIcon = ({ className, color }) => (
  ...
  >
    <path
      d='M6 0L7.90192 4.09808L12 6L7.90192 7.90192L6 12L4.09808 7.90192L0 6L4.09808 4.09808L6 0Z'
      fill={color}
    />
  </svg>
)
export default function BuySubscription() {
  ...
  const handleSubscribe = async () => {
    ...
    if (paymentMethod === 'google_play') {
      ...
      dispatch({
        type: 'vip/initiateGooglePlayPurchase/fulfilled',
        payload: {
    ...
    try {
      ...
      if (response.type && response.type.endsWith('/rejected')) {
        const errorMessage =
          response.payload || 'Payment initiation failed. Please try again.'
      ...
      if (
        ...
        payloadData.includes('already has an active VIP subscription')
      ) {
        alert(
          'You already have an active VIP subscription. No payment is needed!',
        )
        return
      }
      ...
      if (payloadData?.clientSecret && payloadData?.subscriptionId) {
        ...
        if (typeof payloadData === 'string') {
          ...
          alert(
            'Payment initiation failed: Missing payment data. Please try again.',
          )
        }
      }
    } catch (error) {
    ...
    try {
      ...
      dispatch(
        setPurchaseStatus({
          status: 'succeeded',
          message:
            'Payment completed successfully! VIP subscription activated.',
        }),
      )
      ...
      if (
        ...
        errorMessage =
          "Payment processing error: Account configuration issue detected. Your payment was processed, but we need to set up your account. Please contact support with your payment details and we'll activate your subscription."
      }
      ...
      if (
        isNetworkError &&
        !errorMessage.includes('No such customer') &&
        !errorMessage.includes('Account configuration')
      ) {
        // For network errors, show a more user-friendly message
        errorMessage =
          'Network error during payment confirmation. Your payment was successful, but we need to verify it. Please contact support if this persists.'
      } else if (
        isValidationError &&
        !errorMessage.includes('No such customer') &&
        !errorMessage.includes('Account configuration')
  ...
  const handleGooglePlayPaymentSuccess = async () => {
    // Refresh VIP status from backend
    dispatch(fetchVipStatus(token))
    // Fetch active Google Play subscription to sync local state
  ...
  return (
        ...
        <main className='w-full flex flex-col items-center '>
              ...
              >
                {purchaseStatus === 'loading' && (
                  <div className='flex flex-col items-center gap-3'>
                    ...
                    <p className="[font-family:'Poppins',Helvetica] text-sm opacity-90">
                ...
                {purchaseStatus === 'succeeded' && (
                  <div className='flex flex-col items-center gap-3'>
                    ...
                    <div className='text-center'>
                      <p className="text-green-400 [font-family:'Poppins',Helvetica] font-bold text-lg">
                        Payment Completed!
                      </p>
                      <p className="text-white [font-family:'Poppins',Helvetica] text-sm mt-1">
                        VIP subscription activated successfully
                      </p>
                    </div>
                    <button
                      onClick={() => dispatch(resetPurchaseStatus())}
                      className='mt-2 px-6 py-2 bg-green-600 hover:bg-green-700 rounded-lg transition-colors'
                    >
                      Continue
                    </button>
                  </div>
                ...
                {purchaseStatus === 'failed' && (
                  <div className='flex flex-col items-center gap-2'>
                    ...
                    <p className="text-red-400 [font-family:'Poppins',Helvetica] font-medium">
                      {purchaseError || 'Something went wrong.'}
                    </p>
                    <button
                      onClick={() => dispatch(resetPurchaseStatus())}
                      className='mt-1 px-4 py-2 bg-red-600 rounded-md'
                    >
                      Close
                    </button>
                  </div>
          ...
          <header className='relative w-full h-[279px] rounded-b-[50px]'>
            ...
            <div
              ...
              style={{
                background:
                  (currentTierData && currentTierData.gradient) ||
                  'linear-gradient(331deg, #555 0%, #888 100%)',
              }}
            >
              <div className='absolute w-[219px] h-[216px] top-8 left-1/2 -translate-x-1/2'>
                <div className='absolute w-[210px] h-[216px] top-0 left-0'>
                  ...
                  <StarIcon
                    color={currentTierData.iconColor}
                    className='absolute w-[19px] h-[19px] top-[21px] left-[191px]'
                  />
                  <StarIcon
                    color={currentTierData.iconColor}
                    className='absolute w-[19px] h-[19px] top-[197px] left-[156px]'
                  />
                  <SparkleIcon
                    color={currentTierData.iconColor}
                    className='absolute w-[19px] h-[19px] top-2.5 left-[22px]'
                  />
                  <img
                ...
                <SparkleIcon
                  color={currentTierData.iconColor}
                  className='absolute w-[19px] h-[19px] top-[143px] left-[200px]'
                />
              </div>
              <StarIcon
                color={currentTierData.iconColor}
                className='absolute w-[19px] h-[19px] top-[134px] left-[37px]'
              />
              <SparkleSmallIcon
                color={currentTierData.iconColor}
                className='absolute w-3 h-[13px] top-[63px] left-[41px]'
              />
              <SparkleSmallIcon
                color={currentTierData.iconColor}
                className='absolute w-3 h-[13px] top-[194px] left-[321px]'
              />
              <SparkleSmallIcon
                color={currentTierData.iconColor}
                className='absolute w-3 h-[13px] top-[223px] left-[63px]'
              />
              <SparkleIcon
                color={currentTierData.iconColor}
                className='absolute w-[19px] h-[19px] top-[113px] left-[337px]'
              />
            </div>
          </header>
          <nav
          ...
          >
            {membershipTiers.map((tier) => {
              ...
              return (
                <button
                  key={tier.id}
                  className={`inline-flex items-center justify-center gap-2.5 px-[21px] py-[13px] relative flex-1 ${
                    tier.id === 'bronze'
    ...
    ${
      isActive
        ? `font-bold text-[#f4f3fc] text-[18px] text-center ${
            tier.name === 'Platinum' ? 'mr-3' : ''
          }`
    ...
    tracking-[0] leading-[normal]`}
              ...
              {(() => {
                ...
                if (plans.length === 0) {
                  ...
                  return defaultPlans.map((plan) => (
                    <button
                      key={plan.id}
                      className={`flex w-[335px] h-[70.53px] items-center justify-between px-[25.98px] py-[22.27px] relative rounded-[18.56px] ${
      ...
      {paymentMethod === 'stripe' &&
        purchaseResponse &&
        purchaseResponse.data?.clientSecret && (
          <StripePaymentSheet
